...
public sealed record LevelCoverageReport
{
    ...
    private static LevelActorCoverage Classify(LevelActor actor)
    {
        ...
        if (actor.Source.ClassName == "PathNode"
            || HasAny(actor, "PathList", "PathCollisionRadius", "bIsAutoGenerated"))
            return LevelActorCoverage.NavigationPending;
        if (actor.Source.ClassName == "Script" || HasAny(actor, "Actions", "Concepts"))
            return LevelActorCoverage.ScriptPending;
        if (actor.Source.ClassName is "Marker" or "TrainingMarker" or "MapUILayerScaleMarker")
            return LevelActorCoverage.MarkerPending;
