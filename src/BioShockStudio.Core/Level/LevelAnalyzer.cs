...
public static class LevelAnalyzer
{
    ...
    private static readonly IReadOnlySet<string> Interpreted = new HashSet<string>(StringComparer.Ordinal)
    {
        ...
        "DelayBeforeOpening", "StayOpenDuration", "Attachments",
        "ScriptedSequence",
    };
        ...
        Dictionary<string, int> uninterpreted)
    {
        ...
        return new LevelActor
        {
            ...
            ScriptedSequence = ScriptedSequence(package, payload),
    ...
    /// <summary>
    /// A door's <c>Attachments</c> array — mesh-on-socket structs, same
    /// <c>ReadStructArrayElements</c> shape as <c>DamagedReactions</c>. Empty when absent.
    /// <paramref name="complete"/> is false only when the property is present but does not consume
    /// as a struct array.
    /// </summary>
        ...
        BioShockPackage package, ActorPayload payload, out bool complete)
    {
        ...
        var result = new List<DoorAttachmentData>(elements.Count);
        foreach (var fields in elements)
        {
            ...
            if (Find("StaticMesh") is { Type: UnrealPropertyType.Object } meshField
                && PropertyValues.AsReference(meshField) is { IsNull: false } meshIndex)
                mesh = Describe(package, meshIndex, "door attachment", "StaticMesh");

            result.Add(new DoorAttachmentData
            {
                StaticMesh = mesh,
                AttachSocket = Find("AttachSocket") is { Type: UnrealPropertyType.Name } socket
                    ? PropertyValues.AsName(socket, package) : null,
                LocationOffset = Find("AttachLocationOffset") is { } loc
                    ? PropertyValues.AsVector(loc) : null,
                RotationOffset = Find("AttachRotationOffset") is { } rot
                    ? PropertyValues.AsRotator(rot) : null,
                InteractWithPhysicalObjects = Find("InteractWithPhysicalObjects") is { Type: UnrealPropertyType.Bool } flag
                    && flag.BoolValue,
            });
        }
    ...
    /// <summary>
    /// A placed actor's <c>ScriptedSequence</c> array — chained animation clips, not a door field.
    /// Gated on field presence. Nested <c>ScriptedAnimations</c> is a struct array of
    /// <c>Chance</c>+<c>Animation</c> (a name, not an object reference). <c>LoopCount</c> is the
    /// same tagged <c>Range</c> <see cref="ReadFloatRange"/> already reads. <c>RunNext</c> is
    /// carried raw — whether it is a next-entry index is <c>PLAUSIBLE</c> only.
    /// </summary>
    private static ScriptedSequenceActorData? ScriptedSequence(BioShockPackage package, ActorPayload payload)
    {
        if (payload.Find("ScriptedSequence") is not { Type: UnrealPropertyType.Array } property)
            return null;
        if (ReadStructArrayElements(package, property) is not { } elements)
            return new ScriptedSequenceActorData { Entries = [], Complete = false };

        var entries = new List<ScriptedSequenceEntry>(elements.Count);
        bool complete = true;
        foreach (var fields in elements)
        {
            UnrealProperty? Find(string name) => fields.FirstOrDefault(field => field.Name == name);

            FloatRange? loopCount = null;
            if (Find("LoopCount") is { Type: UnrealPropertyType.Struct } loop)
            {
                loopCount = ReadFloatRange(package, loop);
                if (loopCount is null) complete = false;
            }

            IReadOnlyList<ScriptedAnimationChoice> animations = [];
            if (Find("ScriptedAnimations") is { Type: UnrealPropertyType.Array } anims)
            {
                if (ReadStructArrayElements(package, anims) is not { } animElements)
                {
                    complete = false;
                }
                else
                {
                    var choices = new List<ScriptedAnimationChoice>(animElements.Count);
                    foreach (var animFields in animElements)
                    {
                        var chance = animFields.FirstOrDefault(field =>
                            field.Name == "Chance" && field.Type == UnrealPropertyType.Int);
                        var animation = animFields.FirstOrDefault(field =>
                            field.Name == "Animation" && field.Type == UnrealPropertyType.Name);
                        if (animation is null) complete = false;
                        choices.Add(new ScriptedAnimationChoice
                        {
                            Animation = animation is null ? null : PropertyValues.AsName(animation, package),
                            Chance = chance?.AsInt(),
                        });
                    }
                    animations = choices;
            ...
            entries.Add(new ScriptedSequenceEntry
            {
                Animations = animations,
                LoopCount = loopCount,
                RunNext = Find("RunNext") is { Type: UnrealPropertyType.Int } run ? run.AsInt() : null,
                TotalChance = Find("TotalChance") is { Type: UnrealPropertyType.Int } total ? total.AsInt() : null,
            });
        }

        return new ScriptedSequenceActorData { Entries = entries, Complete = complete };
    }
