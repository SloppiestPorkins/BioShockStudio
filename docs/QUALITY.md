...
| 0 | Note | `mesh-materials-without-sections` | **Zero since 22 Aug 2026.** Was 94, and before that 157 and 153. A skeletal mesh naming several materials whose section table did not resolve drew all of itself in one — invisible to every count, since the mesh is complete and merely wearing the wrong paint. The table was only reachable by walking forward from the socket table, so a mesh with no sockets lost it for an unrelated reason; `SkeletalMeshSectionReader` now also counts backward from the bone map, which needs nothing but the geometry. Coverage **331 of 944 exports (35%) → 966 of 967 (99.9%)**, and the two routes produce byte-identical tables wherever both succeed (`SkeletalMeshSectionCoverageTests`). |
