...
**Recently released:** AI-state batch in-world (`run_script_ai_tweak.py`, invincible pawn health 100→100, `Success - 0 error(s)` 28 Aug 2026) — TweakAIVision/Hearing, MuteAI, TellAIToWait/Continue, SetAIPatrol, PostMovementGoal, ToggleAIReactions, SetPawn/PlayerInvincibility, SetTipPriority. ActionGiveItemsToPlayer in-world (`run_script_inventory_ui.py`, ammo stack 3). ActionAttackTarget in-world. Game-mode possess on 1-Medical (`run_game_possess.py`).
