...
3. **Extend the level manifest** to carry the script graph, AI spawner configuration, zone
   ...
   **Manifest half substantially done — audit 28 Aug 2026, see §9.** Spawner config and zone
   membership are in the v4 manifest and pinned by tests; the script graph travels as action
   identities in the manifest plus the full parameter tree in the `export-script-actions` sidecar
   (Medical-complete, 20 maps unrun). Archetype references are carried as raw names — **what they
   resolve to is `UNKNOWN`** and is a separate open investigation (`research/interaction.md` §4),
   not a Phase 2.3 loose end. See §9.
...
### Phase 4 census batch17 (HeadTrack / Avoidance / RemoveItems / LevelSwitch) — done, 26 Aug 2026
...
### Phase 4 census batch19 (Vita / Holdable / Achievement / WeaponFire) — done, 26 Aug 2026

Live `run_action_batch19.py`, `Success - 0 error(s)`. Station enable /
holdable inventory / achievements / weapon-fire messages still open. Next: keep
walking the census head.

### Playable-slice stand-ins + census batch20–24 — verified, 26 Aug 2026

Live UE5.7 after BuildPlugin (Fire bind void-handler fix):

- `run_playable_standin.py` — `Success - 0 error(s)`: HP damage, hitscan hit,
  `SpawnAI.SpawnAtLocation`, `AttackTarget.ApplyImmediateDamage`.
- `run_action_batch20.py` … `run_action_batch24.py` — all `Success - 0 error(s)`.
...
### Phase 4 census batch33–34 (FxWait / Grenadier / CritWait / HUD / SecBot / DLC) — done, 26 Aug 2026

Live `run_action_batch33.py` + `run_action_batch34.py`, `Success - 0 error(s)`.
FxWait + CritWait carry 60s schema TimeoutSeconds default.
Next: EnableBathysphereModeForPlayer / WaitUntilActorHasLanded.

### Phase 4 census batch35–36 (BathMode / Landed / WaterVol / GathererLbl / CollListen / HavokEnable / Ragdoll / AssTp) — done, 27 Aug 2026

Live `run_action_batch35.py` + `run_action_batch36.py`, `Success - 0 error(s)`.
...
`AddLoopAction` / `AddTest` / `AddForAction`. Medical dump: 1958 unique exports,
495 nested enqueued. Live import (limit 70): nested_for **3**, nested_unmapped **0**.
