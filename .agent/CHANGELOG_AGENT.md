...
|---|---|---|---|---|---|
| 2026-08-23 | TASK-002 | Lead (decompiled `IGSoundEffectsSubsystem.U` directly — needed real code execution, outside the current Research agent's read-only capability) | `KNOWN_ASSUMPTIONS.md` A18, `OPEN_QUESTIONS.md` Q2 | evidence only (no code change) — `SoundSpecEntry.Flag` confirmed as `Material.EMaterialVisualType`, full 28-value enum found and cross-referenced against `Bioshock1REMSDK-WIP--main`; A18 promoted `PLAUSIBLE` → `HIGH`. Follow-up: TASK-010. | — |
| 2026-08-23 | TASK-003 | local research agent (unreliable first pass) → Lead (corrected) | `.agent/TASK_QUEUE.md` | **partial** — the worker's grep-based classification of census vs. single-package tests was wrong (misclassified `StaticMeshGeometryTests.cs`); Lead re-verified and recorded the one concrete instance (`KNOWN_ASSUMPTIONS.md` A9) rather than claim false completeness | — |
| 2026-08-23 | TASK-009 | Lead (gate tracing across 9 buckets) → local coder → local tester → local reviewer → Lead (integration) | `tests/BioShockStudio.Tests/{AntiPortal,CubemapProbe,Spawner}ActorSchemaTests.cs`, `OPEN_QUESTIONS.md` Q11 | **accepted** — 3 more tests hardened against the same shared-bucket pattern before they broke, not after; `SoundActorSchemaTests` deliberately left open (ambiguous author intent, needs Research); `ProjectorActorSchemaTests`/`HavokConstraintActorSchemaTests`/`EffectActorSchemaTests` confirmed already safe | (uncommitted) |
| 2026-08-23 | — | Lead | `.agent-control/agent.ps1` — non-interactive local-agent orchestration layer built and smoke-tested (real Ollama dispatch, not the `claude` CLI — see `CLAUDE_ORCHESTRATION.md` for why) | accepted | `a06779e` |
| 2026-08-23 | TASK-000 | Lead (research) → local coder → local tester → local reviewer → Lead (integration) | `tests/BioShockStudio.Tests/{Marker,Pickup,Vending}ActorSchemaTests.cs`, `TrainingScriptActorTests.cs` | **accepted** — shared coverage-bucket test-design defect fixed, no production code touched; verified 5/5 in an isolated worktree and again in main | `43dcaaa` |
